...
/// Ниже - блок кода для тестирования операций с векторами
...
static const char* dotProductExpectedResult = ":+,+,*,x1,y1,*,x2,y2,*,x3,y3";
static const char* sumProductExpectedResult[3] = { ":+,x1,y1", ":+,x2,y2", ":+,x3,y3" };
...
static const FieldInfo fakeOperationsService = { &sumNodeFake, &multiplyNodeFake, &getNodeValueFake, &printNodeFake, sizeof(struct Node) }; // почему-то getNodeSize() не принимает
...
void setVectorFakeInfo(Vector* vector) {
    ...
    vector -> fieldInfo = setFakeOperationsService(); // Только ради этого мы вынесли объявление Vector в Vector.h и подключили его к двум .cpp файлам
...
void printToBuff(char **buff, const char separator, int *remainingSize, NodePtr node) {
    assert(node); // для первого вызова функции
    assert(node -> lexem);

    int stepLength = 1 + strlen(node -> lexem); // separator + lexem
...
void testDotProduct(void) {
    ...
    // Но надо учитывать, что getValue(в fieldInfo) в этом случае ничего не делает, поэтому мы должны сами проинициализировать вектор
    ...
    assert(strcmp(buff, dotProductExpectedResult) == 0);
...
void testSumProduct(void) {
    ...
    nodeForInit -> lexem = X3;
    setElementValue(firstVector, 3, (void*) nodeForInit);
    //printf("%s\n", ((NodePtr)(getElementPtr(firstVector, 2))) -> lexem); // печатает строку, которая содержится в Node.
    ...
    for (int i = 1; i <= SIZE; i++) {
        ...
        assert(strcmp(buff[i - 1], sumProductExpectedResult[i - 1]) == 0);
    }
    free(sumProduct);
